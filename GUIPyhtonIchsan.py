...
def AddForm():
    ...
    tk.Label(win, text="Waktu :").grid(row=0, column=0, padx=3, pady=3)
    ...
    tk.Label(win, text="Judul :").grid(row=1, column=0, padx=3, pady=3)
    tk.Entry(win, textvariable=judul).grid(row=1, column=1, columnspan=2)
    tk.Label(win, text="Keterangan :").grid(row=2, column=0)
    keterangan = ScrolledText(win, width=12, height=5)
    keterangan.grid(row=2, column=1, columnspan=2, rowspan=4, padx=3, pady=3)
    tanggal = str(cal.selection_get())
    tk.Button(win, text="Tambah", command=lambda: addTodo(win, tanggal, jam, menit, judul, keterangan)).grid(row=6, column=0, sticky="E", columnspan=3, padx=2, pady=2)
...
cal = Calendar(root, font="Arial 14 bold", selectmode="day", locale="id_ID", cursor="hand1", background="#07575B", headersbackground="#66A5AD", headersforeground="White", selectbackground="#07575B")
...
scrollBar.grid(row =0, column=3, sticky="ENS", rowspan=7)
...
btnAdd = tk.Button(root, text="Tambah", width=20, background="#003B46", command=AddForm)
btnAdd.grid(row=4, column=1, sticky="N")

btnDel = tk.Button(root, text="Hapus", width=20, background="#07575B", command=delTodo)
btnDel.grid(row=4, column=2, sticky="N")

btnLoad = tk.Button(root, text="Load", width=20, background="#66A5AD", command=LoadTodos)
btnLoad.grid(row=6, column=1, sticky="S")

btnSave = tk.Button(root, text="Save", width=20, background="#1995AD", command=SaveTodos)
